<template>
    <div>
            ...
            <div class="content">
                ...
                <el-form :model="productSetform" :rules="productSetformRules" ref="formRef" label-width="100px">
                    ...
                    <el-form-item label="序号" prop="order">
                        <el-input v-model="productSetform.order" :disabled="isEdit" placeholder="请输入序号" />
                    </el-form-item>

                    <el-form-item label="分类名称" prop="categoryName">
                        <el-input v-model="productSetform.categoryName" placeholder="请输入分类名称" />
                    </el-form-item>
        ...
        <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :title="productTitle" :visible.sync="isProductCateEditDialog" width="50%" height="400px">
            ...
            <el-button style="width: 100%; margin-top: 40px" type="primary" @click="addRow">添加一行</el-button>

            <span slot="footer" class="dialog-footer" style="display: flex; justify-content: center">
                <el-button type="primary" @click="isProductCateEditDialog = false">返回</el-button>
        ...
        <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" title="编辑下载渠道" :visible.sync="isProductMiddleDowndialog" width="50%" height="400px">
            <div class="content" style="display: flex; flex-direction: column; gap: 10px">
                <div v-for="(item, index) in downloadfrom" style="display: flex; gap: 10px" :key="index">
                    ...
                    <el-input v-model="item.downloadUrl" style="flex: 2" placeholder="请输入下载链接" />
                    <el-button type="danger" @click="middleremoveRow(index)">删除</el-button>
                </div>
            </div>

            <el-button style="width: 100%; margin-top: 40px" type="primary" @click="middleaddRow">添加一行</el-button>

            <span slot="footer" class="dialog-footer" style="display: flex; justify-content: center">
                <el-button type="primary" @click="isProductMiddleDowndialog = false">返回</el-button>
            ...
            <div class="productapp_content" style="min-height: 500px">
                <el-form ref="productForm" :model="productMiddleSetform" :rules="productMiddleSetformRules">
                    ...
                    <el-form-item label="产品名称" prop="name" class="form-item">
                        <el-input v-model="productMiddleSetform.name" placeholder="请输入产品名称" />
                    </el-form-item>
                    ...
                    <el-form-item label="产品LOGO" prop="logo">
                        ...
                        <el-upload ref="upload" action="action" :file-list="logoList" :on-change="handleuploadLogo" :on-remove="handleuploadLogoRemove" :limit="1" :auto-upload="false">
                            <el-button slot="trigger" size="small" type="primary" v-if="!middlelogobase64">上传图片</el-button>
                            <el-button slot="trigger" size="small" type="danger" v-if="middlelogobase64" @click="handleReplaceClick">替换图片</el-button>
                        </el-upload>
                    </el-form-item>

                    <!-- 下载渠道 -->
                    <el-form-item label="下载渠道" prop="download">
                        <el-input :disabled="true" style="width: 40vh" v-model="productMiddleSetform.download" />
                        <el-button type="primary" @click="handleDownloadEdit()">编辑</el-button>
                    </el-form-item>
                    ...
                    <el-form-item label="UI背景图">
                        ...
                        <el-upload ref="upload2" action="action" :file-list="bgList" :on-change="handleuploadBg" :on-remove="handleuploadBgRemove" :limit="1" :auto-upload="false">
                            <el-button slot="trigger" v-if="!middlebackgroupbase64" size="small" type="primary">+ 添加图片</el-button>
                            <el-button slot="trigger" v-if="middlebackgroupbase64" size="small" type="danger" @click="handleReplaceClick2">替换图片</el-button>
                        </el-upload>
                    </el-form-item>
                    <el-form-item label="移动端UI背景图">
                        <el-input v-if="middleMbackgroupbase64" v-model="middleMbackgroupbase64" width="300px" />
                        <el-upload ref="upload2" :show-file-list="false" action="action" :on-change="handleuploadMBg" :on-remove="handleuploadMBgRemove" :limit="1" :auto-upload="false">
                            <el-button slot="trigger" v-if="!middleMbackgroupbase64" size="small" type="primary">+ 添加图片</el-button>
                            <el-button slot="trigger" v-if="middleMbackgroupbase64" size="small" type="danger">替换图片</el-button>
                        </el-upload>
                    </el-form-item>
                </el-form>
            </div>
        ...
        <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" title="推荐产品分类" :visible.sync="isproductPreview" width="80%">
            ...
            <div class="preview_footer" style="display: flex; justify-content: center; gap: 20px">
                <el-button type="warning" @click="handlePreviewcenter">产品中台</el-button>
                <el-button type="primary" @click="previewcenterselectall">全部选中</el-button>
                <el-button type="danger" @click="previewcenterdelete">删除所选</el-button>
                <el-button type="primary" @click="isproductPreview = false">保存</el-button>
            </div>
        </el-dialog>
        ...
        <div v-if="!isProductcenter">
            <div class="header">
                <div class="product_selectbox">
                    ...
                    <el-button type="warning" style="height: 30px" @click="handlereset()">重置</el-button>
                </div>

                <div class="setbox">
                    <el-button type="primary" @click="addProductCategory('add')">+ 新增分类</el-button>
                    <el-button type="warning" @click="handleProductcenter">产品中台</el-button>
                </div>
            </div>
            ...
            <div class="main">
                <el-table :data="tableData" border :key="1" stripe :header-cell-class-name="'headerBg'" style="width: 100%">
                    ...
                    <el-table-column prop="international" label="国际化"> </el-table-column>
                    <el-table-column prop="updateTime" label="最近更新时间"> </el-table-column>
                    <el-table-column prop="actions" label="操作" width="280">
                        <template v-slot="scope">
                            <!-- 查看分类按钮 -->
                            <el-button type="primary" @click="handlePreview(scope.row)"> 查看分类 </el-button>

                            <!-- 编辑分类按钮 -->
                            <el-button type="primary" @click="addProductCategory('edit', scope.row)"> 编辑分类 </el-button>

                            <!-- 删除按钮 -->
                            <el-button type="danger" @click="delProductCategory(scope.row)">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
        ...
        <div v-if="isProductcenter">
            <div class="headers">
                ...
                <div class="setbox">
                    <el-button type="primary" @click="handleProductcenteradd('add')">+ 新增产品</el-button>
                </div>
                <img src="@/assets/dist/images/close.svg" class="close" @click="handleProductcentercolse" alt="" />
            </div>
            <div class="contentbox">
                <el-table :data="tableDatas" :row-style="{ height: '35px' }" :key="2" stripe style="width: 100%">
                    <el-table-column prop="sortIndex" label="序号" width="80"></el-table-column>
                    <el-table-column prop="name" label="产品名称" width="180"></el-table-column>
                    <el-table-column prop="info" label="产品简介"></el-table-column>
                    <el-table-column prop="logo" label="产品LOGO">
                    ...
                    <el-table-column prop="actions" label="操作" width="280">
                        <template v-slot="scope">
                            ...
                            <el-button type="danger" @click="handleProductcenterdelete(scope.row)">删除</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
export default {
    data() {
        return {
            ...
            productSetformRules: {
                website: [{ required: true, message: '请选择所属官网', trigger: 'blur' }],
    ...
    methods: {
        ...
        addProductCategory(type, row) {
            ...
            if (type === 'add') {
                ...
                this.isEdit = false
                this.request.post('/product/productInfo', { id: row.id }).then(res => {
                    console.log(res)
                    if (res.code === '200') {
                        this.productSetform = {
        ...
        handleEdit(type) {
            ...
            if (type === 'productCategoryEdit') {
                ...
                if (this.productMiddleSetform.internationalName) {
                    this.Localizationfrom = JSON.parse(this.productMiddleSetform.internationalName)
                } else {
                    this.Localizationfrom = [{ language: '', content: '' }]
                }
                ...
                if (this.productMiddleSetform.internationalInfo) {
                    this.Localizationfrom = JSON.parse(this.productMiddleSetform.internationalInfo)
                } else {
                    this.Localizationfrom = [{ language: '', content: '' }]
                }
                ...
                if (this.productMiddleSetform.websiteUrl) {
                    this.Localizationfrom = JSON.parse(this.productMiddleSetform.websiteUrl)
                } else {
                    this.Localizationfrom = [{ language: '', content: '' }]
                }
            }
        ...
        saveLocalization() {
            ...
            if (this.LocalizationType === 'productCategoryEdit') {
                ...
                this.productSetform.localization = JSON.stringify(this.Localizationfrom)
            } else if (this.LocalizationType === 'productNameEdit') {
                this.isProductCateEditDialog = false
                this.productMiddleSetform.internationalName = JSON.stringify(this.Localizationfrom)
            } else if (this.LocalizationType === 'productDetailEdit') {
                this.isProductCateEditDialog = false
                this.productMiddleSetform.internationalInfo = JSON.stringify(this.Localizationfrom)
            } else if (this.LocalizationType === 'productWebsiteEdit') {
                this.isProductCateEditDialog = false
                this.productMiddleSetform.websiteUrl = JSON.stringify(this.Localizationfrom)
            }
        },
        //保存下载渠道
        saveDownload() {
            ...
            if (type === 'add') {
                ...
                this.request.post('/middle/middleInfo', { id: row.id }).then(res => {
                    if (res.code === '200') {
                        ...
                        this.middleMbackgroupbase64 = res.data.middleInfo.ui_back_ground_mobile
                    }
                })
            }
        },
        ...
        async handleuploadBg(file) {
            this.middlebackgroupbase64 = await this.fileToBase64(file.raw).then(res => {
                return res
            })
        },
        async handleuploadMBg(file) {
            this.middleMbackgroupbase64 = await this.fileToBase64(file.raw).then(res => {
                return res
            })
        },
