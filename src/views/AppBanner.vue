<template>
    <div>
        ...
        <div style="padding: 10px 0">
            ...
            <el-button type="primary" @click="templateAddEn" style="text-align: right;">表格国外模板图片<i class="el-icon-circle-plus-outline"></i></el-button>
            <el-button type="primary" @click="templateAddZh" style="text-align: right;">表格国内模板图片<i class="el-icon-circle-plus-outline"></i></el-button>
        </div>
        <el-table :data="tableData" style="width: 100%" :key="itemKey" border stripe :header-cell-class-name="'headerBg'" @selection-change="handleSelectionChange">
        ...
        <el-dialog title="新增轮播图信息" :visible.sync="dialogFormVisible" width="30%">
            <el-form label-width="80px" size="small" :model="form" ref="addForm">
                ...
                <el-form-item label="轮播图文本" prop="bannerText" :rules="{
                            required: true, message: '请填写轮播图文本', trigger: 'blur'
                        }">
        ...
        <el-dialog title="修改轮播图信息" :visible.sync="dialogFormVisible1" width="30%">
            <el-form label-width="80px" size="small" :model="form" ref="editForm">
                ...
                <el-form-item label="轮播图文本" prop="bannerText" :rules="{
                            required: true, message: '轮播图文本不能为空', trigger: 'blur'
                        }">
                    <el-input v-model="form.bannerText" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="轮播图">
                ...
                <el-form-item label="轮播时长" prop="bannerTime" :rules="{
                            required: true, message: '请默认分配轮播时长', trigger: 'blur'
                        }">
                    <el-input v-model="form.bannerTime" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="轮播存放点" prop="sortIndex" :rules="{
                            required: true, message: '请分配默认存放点', trigger: 'blur'
                        }">
                    <el-input type="number" v-model="form.sortIndex" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="跳转链接" prop="bannerUrl">
        ...
        <el-dialog title="表格模板图片" :visible.sync="templateDialog">
            ...
            <div class="imgList">
                <div class="img-item" v-for="(item, index) in formUrlList" :class="{active:item.bOn}" :key="index" @click="checkImg(index)">
                    <p style="margin-bottom: 5px; text-align: center; font-weight: bold; font-size: 14px;">{{ item.name }}</p>
    ...
    <script>
        ...
        export default {
            ...
            watch: {
                formUrlText(newVal) {
                    if (newVal != undefined && newVal != "") {
                        this.request.post("/ossImage/searchImageUrlList", {
                        ...
                        this.request.post("/ossImage/ImageUrlList", {
                            "webMark": this.webMark
                        }).then(res => {
                            if (res.code == 200) { 
                                this.formUrlList = []
            ...
            methods:{
                load(){
                    this.request.get("/bannerManage/bannerList", {
                        params: {
                ...
                save(){
                    this.$refs.addForm.validate(valid => {
                        if (valid) {
                            ...
                            this.request.post("/bannerManage/saveBanner",this.form).then( res =>{
                                if (res.code === '200'){
                ...
                edit(){
                    this.$refs.editForm.validate(valid => {
                        if (valid) {
                            ...
                            this.request.post("/bannerManage/saveBanner",this.form, {
                ...
                handleDelete(id){
                    this.request.post("/bannerManage/deleteBanner", {
                        id
                    }).then(res => {
                        if (res.code === '200'){
                ...
                search() {
                    this.request.post("/bannerManage/searchBanner", {
                            pageNum: this.pageNum,
                            pageSize: this.pageSize,
                ...
                innerVisibleAdd() {
                    this.$refs.mustForm.validate(valid => {
                        if (valid) {
                            ...
                            this.request.post("/bannerManage/saveBanners", this.mustForm).then(res => {
                                if (res.code == 200) {
                                    this.$message.success("保存成功")
