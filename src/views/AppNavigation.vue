<template>
    <div>
        ...
        <el-table :data="tableData" :key="itemKey" style="width: 100%" border stripe :header-cell-class-name="'headerBg'">
            ...
            <el-table-column prop="navigationName" label="自媒体名称" width="150" align="center"></el-table-column>
            <el-table-column prop="navigationUrl" label="自媒体跳转链接" width="150" align="center"></el-table-column>
            <el-table-column label="图标" width="150" align="center">
                <template slot-scope="scope">
                    <img v-if="scope.row.navigationIcon" :src="'//' + scope.row.navigationIcon" alt="" style="width: 50px; height: 50px"/>
            ...
            <el-table-column label="二维码" width="150" align="center">
                <template slot-scope="scope">
                    <img v-if="scope.row.navigationImg" :src="'//' + scope.row.navigationImg" alt="" style="width: 50px; height: 50px"/>
                    <el-progress v-if="!uploadData && scope.row.id == currentId" :percentage="uploadProgress"></el-progress>
                </template>
            </el-table-column>
        ...
        <el-dialog title="新增自媒体" :visible.sync="dialogFormVisible" width="30%">
            <el-form label-width="80px" size="small" :model="form" ref="addForm">
                <el-form-item label="自媒体名称" prop="navigationName" :rules="{
                            required: true, message: '自媒体名称不能为空', trigger: 'blur'
                        }">
                    <el-input v-model="form.navigationName" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="跳转链接" prop="navigationUrl" :rules="{
                            required: true, message: '跳转链接不能为空', trigger: 'blur'
                        }">
                    <el-input v-model="form.navigationUrl" autocomplete="off"></el-input>
                </el-form-item>
        ...
        <el-dialog title="修改自媒体信息" :visible.sync="dialogFormVisible1" width="30%">
            <el-form label-width="80px" size="small" :model="form" ref="editForm">
                <el-form-item label="自媒体ID" v-if="false">
                ...
                <el-form-item label="图标">
                    <el-input v-model="form.navigationIcon" autocomplete="off" v-if="form.navigationIcon"></el-input>
                    <el-upload
                        class="upload-demo"
                        ref="upload"
                        action="action"
                ...
                <el-form-item label="二维码">
                    <el-input v-model="form.navigationImg" autocomplete="off" v-if="form.navigationImg"></el-input>
                    <el-upload
                        class="upload-demo"
                        ref="upload1"
                        action="action"
    ...
    <script>
        ...
        export default {
            ...
            methods:{
                load(){
                    this.request.get("/ngManage/ngManageList", {
                        params: {
                        ...
                        this.tableData = res.data.ngManageList.records
                        this.total = res.data.ngManageList.total
                ...
                save(){
                    this.$refs.addForm.validate(valid => {
                        if (valid) {
                            if (this.base64Img) {
                                this.form["navigationIcon"] = this.base64Img
                            } else {
                                this.form["navigationIcon"] = this.form.navigationIcon
                            }
                            if (this.base64Img1) {
                                this.form["navigationImg"] = this.base64Img1
                            } else {
                                this.form["navigationImg"] = this.form.navigationImg
                            }
                            this.request.post("/ngManage/saveNgManage", this.form).then( res =>{
                                if (res.code === '200'){
                ...
                handleEdit(row){
                    ...
                    this.image1 = this.form.navigationImg
                    this.currentId = this.form.id
                    this.dialogFormVisible1 = true
                },
                handleDelete(id){
                    this.request.post("/ngManage/deleteNgManage", {
                        id
                    }).then(res => {
                        if (res.code === '200'){
                ...
                async handlePreview1(file) {
                    this.base64Img1 = await this.fileToBase64(file.raw).then(res => {
                        return res
                    })
                },
                ...
                search() {
                    this.request.post("/ngManage/searchNgManage", {
                            pageNum: this.pageNum,
                            pageSize: this.pageSize,
